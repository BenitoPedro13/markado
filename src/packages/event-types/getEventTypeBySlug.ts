...
import {SchedulingType, MembershipRole} from '~/prisma/enums';
...
export const getEventTypeBySlug = async ({
  ...
  const eventType = {
    ...
    schedule:
      ...
      rawEventType.users[0]?.defaultScheduleId ||
      null,
      ...
      ch.owner !== null
        ? {
            ...
            owner: {
              ...
              membership:
                restEventType.team?.members.find(
                  (tm) => tm.user.id === ch.owner?.id
  ...
  const eventTypeUsers: ((typeof eventType.users)[number] & {
    avatar: string;
  ...
  const currentUserMembership =
    eventTypeObject.team?.members.find((el) => el.user.id === userId) ?? null;
