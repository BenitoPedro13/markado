...
import { DailyLocationType } from "@/packages/core/location";
// import { sendBrokenIntegrationEmail } from "@/emails";
import { getUid } from "@/packages/lib/CalEventParser";
import logger from "@/packages/lib/logger";
import { getPiiFreeCalendarEvent, getPiiFreeCredential } from "@/packages/lib/piiFreeData";
import { safeStringify } from "@/lib/safeStringify";
import { prisma } from "@/lib/prisma";
import type { GetRecordingsResponseSchema } from "~/prisma/zod-utils";
import type { CalendarEvent, EventBusyDate } from "@/types/Calendar";
import type { CredentialPayload } from "@/packages/types/Credential";
import type { EventResult, PartialReference } from "@/packages/types/EventManager";
import type { VideoApiAdapter, VideoApiAdapterFactory, VideoCallData } from "@/packages/types/VideoApiAdapter";
...
const getVideoAdapters = async (withCredentials: CredentialPayload[]): Promise<VideoApiAdapter[]> => {
  ...
  for (const cred of withCredentials) {
    ...
    if (
      "lib" in app &&
      typeof app.lib === "object" &&
      app.lib !== null &&
      "VideoApiAdapter" in app.lib
    ) {
      const makeVideoApiAdapter = (app.lib as { VideoApiAdapter: VideoApiAdapterFactory }).VideoApiAdapter;
