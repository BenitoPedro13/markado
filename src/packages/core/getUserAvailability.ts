...
import type {DateOverride, WorkingHours} from '@/packages/lib/date-ranges';
import {buildDateRanges, subtract} from '@/packages/lib/date-ranges';
...
import {EventTypeMetaDataSchema, stringToDayjsZod} from '~/prisma/zod-utils';
import type {EventBusyDetails, IntervalLimitUnit} from '@/types/Calendar';
import type {TimeRange} from '@/types/scheadule';
...
const getUser = async (
  ...args: Parameters<typeof _getUser>
): Promise<ReturnType<typeof _getUser>> => {
  ...
  return bookings.map((booking) => {
    const attendees = isTeamEvent
      ? booking.attendees.filter(
          (attendee) => !hostEmails?.includes(attendee.email)
        )
      : booking.attendees;
...
export const getUserAvailability = async (
  ...
  return _getUserAvailability(...args)
};

/** This should be called getUsersWorkingHoursAndBusySlots (...and remaining seats, and final timezone) */
const _getUserAvailability =
  async function getUsersWorkingHoursLifeTheUniverseAndEverythingElse(
    query: {
      withSource?: boolean;
      username?: string;
      userId?: string;
    ...
    initialData?: {
      ...
      currentBookings?: (Pick<
        Booking,
        'id' | 'uid' | 'userId' | 'startTime' | 'endTime' | 'title'
      > & {
        eventType: Pick<
          PrismaEventType,
    ...
    if (!dateFrom.isValid() || !dateTo.isValid()) {
      throw new Error('Invalid time range given.');
    }
    ...
    if (!user) throw new Error('No user found in getUserAvailability');
    console.debug(
    ...
    const userSchedule = user.schedules.filter(
      (schedule) =>
        !user?.defaultScheduleId || schedule.id === user?.defaultScheduleId
    )[0];

    const hostSchedule = eventType?.hosts?.find(
      (host) => host.user.id === user.id
    )?.schedule;

    // TODO: It uses default timezone of user. Should we use timezone of team ?
    const fallbackTimezoneIfScheduleIsMissing =
      eventType?.timeZone || user.timeZone;
    ...
    const teamBookingLimits = parseBookingLimit(
    ...
    if (
      ...
      throw new Error('Availability not found for this schedule');
    }
    ...
    if (returnDateOverrides) {
      // const calculateDateOverridesSpan = Sentry.startInactiveSpan({
      //   name: 'calculateDateOverrides'
      // });
      const availabilityWithDates = availability.filter(
      ...
      for (let i = 0; i < availabilityWithDates.length; i++) {
        ...
        const overrideStartDate = dayjs
          .utc(override.date)
          .hour(startTime.hour())
          .minute(startTime.minute());
        const overrideEndDate = dayjs
          .utc(override.date)
          .hour(endTime.hour())
    ...
    const dateRangesInWhichUserIsAvailable = subtract(
      dateRanges,
      formattedBusyTimes
    );
    const dateRangesInWhichUserIsAvailableWithoutOOO = subtract(
...
export const getPeriodStartDatesBetween = (
  ...
  return _getPeriodStartDatesBetween(...args);
  // monitorCallbackSync(
  //   _getPeriodStartDatesBetween, ...args
  // );
};
...
const calculateOutOfOfficeRanges = (
  ...
  return outOfOfficeDays.reduce(
    (acc: IOutOfOfficeData, {start, end, toUser, user, reason}) => {
      // here we should use startDate or today if start is before today
      // consider timezone in start and end date range
      const startDateRange = dayjs(start)
        .utc()
        .isBefore(dayjs().startOf('day').utc())
      ...
      for (
        let date = startDateRange;
        date.isBefore(endDateRange);
        date = date.add(1, 'day')
      ) {
...
type GetUserAvailabilityInitialData = NonNullable<
  Parameters<typeof getUserAvailability>[1]
>;
export type GetAvailabilityUser = NonNullable<
  GetUserAvailabilityInitialData['user']
>;
...
export const getUsersAvailability = async (
  ...
  return _getUsersAvailability(...args)
};
