...
import { MARKADO_API_VERSION_HEADER, X_MARKADO_CLIENT_ID } from "@/packages/platform/constants";
...
const http = (function () {
  ...
  return {
    ...
    setClientIdHeader: (clientId: string) => {
      instance.defaults.headers.common[X_MARKADO_CLIENT_ID] = clientId;
    },
    getClientIdHeader: () => {
      return (
        instance.defaults.headers.common?.[X_MARKADO_CLIENT_ID]?.toString() ??
        ''
      );
    },
    setVersionHeader: (clientId: string) => {
      instance.defaults.headers.common[MARKADO_API_VERSION_HEADER] = clientId;
    },
    getVersionHeader: () => {
      return (
        instance.defaults.headers.common?.[X_MARKADO_CLIENT_ID]?.toString() ??
