...
import { getSuccessPageLocationMessage } from "@/core/locations";
import dayjs from "@/lib/dayjs";
// import { sdkActionManager, useIsEmbed } from "@/packages/embed-core/embed-iframe";
...
import { APP_NAME, WEBAPP_URL } from "@/constants";
...
//   () =>
//     import("@/packages/app-store/paypal/components/PaypalPaymentComponent").then(
...
//   () => import("@/packages/app-store/alby/components/AlbyPaymentComponent").then((m) => m.AlbyPaymentComponent),
//   {
//     ssr: false,
//   }
// );
            ...
            <div className="inset-0 my-4 transition-opacity sm:my-0" aria-hidden="true">
              ...
              <div
                className={classNames(
                  "main bg-default border-soft-200 inline-block transform overflow-hidden rounded-3xl border p-6 text-left align-bottom transition-all  sm:w-full sm:max-w-lg sm:py-6 sm:align-middle",
                  isEmbed ? "" : "sm:my-8"
                )}
                role="dialog"
                aria-modal="true"
                ...
                <div>
                  ...
                  <div className="mt-3 text-center sm:mt-5">
                    ...
                    <div className="text-default border-soft-200 mt-4 grid grid-cols-3 border-t py-4 text-left">
                ...
                <div>
                  {props.payment.success && !props.payment.refunded && (
                    <div className="text-default mt-4 text-center ">{t("paid")}</div>
                  )}
                  ...
                  {props.payment.refunded && (
                    <div className="text-default mt-4 text-center ">{t("refunded")}</div>
                  )}
                </div>
                {!props.profile.hideBranding && (
                  <div className="text-muted  mt-4 border-t pt-4 text-center text-xs">
                    <a href={`${WEBAPP_URL}/signup`}>
                      {t("create_booking_link_with_markado", { appName: APP_NAME })}
                    </a>
                  </div>
                )}
              </div>
