import type { Booking, EventType } from "~/prisma/app/generated/prisma/client";
import {Prisma} from '~/prisma/app/generated/prisma/client';
// import * as Sentry from "@sentry/nextjs";

import { getBusyCalendarTimes } from "@/packages/core/CalendarManager";
import dayjs from "@/lib/dayjs";
import { subtract } from "@/packages/lib/date-ranges";
import { intervalLimitKeyToUnit } from "@/packages/lib/intervalLimit";
// import logger from "@/lib/logger";
import { getPiiFreeBooking } from "@/packages/lib/piiFreeData";
// import { performance } from "@/lib/server/perfObserver";
import {prisma} from "@/lib/prisma";
import type { SelectedCalendar } from "~/prisma/app/generated/prisma/client";
...
export async function getBusyTimes(params: {
  ...
  console.info(
    `Checking Busy time from Markado Bookings in range ${startTime} to ${endTime} for input ${JSON.stringify({
      userId,
      eventTypeId,
  ...
  console.debug(
    `Busy Time from Markado Bookings ${JSON.stringify({
      busyTimes,
...
export async function getBusyTimesForLimitChecks(params: {
  ...
  console.info(`Fetch limit checks bookings for eventId: ${eventTypeId} ${JSON.stringify(busyTimes)}`);
