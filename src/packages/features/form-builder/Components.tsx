...
import {CheckboxField} from '@/packages/ui/form/checkbox';
import * as Label from "@/components/align-ui/ui/label";
import {InfoBadge} from "@/components/align-ui/ui/info-badge";
...
export const isValidValueProp: Record<
  Component['propsType'],
  (val: unknown) => boolean
> = {
  ...
  objectiveWithInput: (val) =>
    typeof val === 'object' && val !== null ? 'value' in val : false,
  select: (val) => typeof val === 'string',
  text: (val) => typeof val === 'string',
  textList: (val) =>
    val instanceof Array && val.every((v) => typeof v === 'string'),
  variants: (val) =>
    (typeof val === 'object' && val !== null) || typeof val === 'string'
};

type Component =
  | {
      propsType: 'text';
      factory: <TProps extends TextLikeComponentProps>(
        props: TProps
      ) => JSX.Element;
    }
  | {
      propsType: 'textList';
      factory: <TProps extends TextLikeComponentProps<string[]>>(
        props: TProps
      ) => JSX.Element;
    }
  | {
...
export const Components: Record<FieldType, Component> = {
  text: {
    ...
    factory: (props) => (
      <Widgets.TextWidget id={props.name} noLabel={true} {...props} />
    )
  },
  textarea: {
    ...
    factory: (props) => (
      <Widgets.TextAreaWidget id={props.name} rows={3} {...props} />
    )
  },
  number: {
    propsType: propsTypes.number,
    factory: (props) => (
      <Widgets.NumberWidget id={props.name} noLabel={true} {...props} />
    )
  },
  name: {
    ...
    factory: (props) => {
      ...
      if (variantName !== 'firstAndLastName' && variantName !== 'fullName') {
        throw new Error(
          `Invalid variant name '${variantName}' for 'name' type of field`
        );
      }

      const value = preprocessNameFieldDataWithVariant(
        variantName,
        props.value
      );
  ...
  radioInput: {
    ...
    factory: function RadioInputWithLabel({
      ...
      const didUserProvideLabel = (
        ...
        return label && translatedDefaultLabel
          ? translatedDefaultLabel !== label
          : false;
      };
      ...
      return (
        <div>
                  ...
                  return (
                    <label key={i} className="mb-1 flex items-center">
                      ...
                      <span>
                        {option.value === 'phone' && (
                          <InfoBadge
                            content={t('number_in_international_format')}
                          />
                        )}
                      </span>
                    </label>
                ...
                <>
                  <Label.Root className="flex items-center">
                    ...
                    {options[0].value === 'somewhereElse'
                      ? translatedDefaultLabel
                      : getCleanLabel(
                          didUserProvideLabel(label, translatedDefaultLabel)
                            ? label
                            : options[0].label
                        )}
                    {!readOnly && optionsInputs[options[0].value]?.required ? (
                      <span className="text-default -mb-2 ml-1 text-sm font-medium">
                        *
                      </span>
                    ) : null}
          ...
          {(() => {
            const optionField = optionsInputs[value?.value as keyof typeof optionsInputs];
            if (!optionField) {
              return null;
            }
            return (
