...
// import type { routingFormResponseInDbSchema } from "@/packages/app-store/routing-forms/zod";
...
type CreateBookingParams = {
  ...
  reroutingFormResponses: unknown | null;
  ...
  if (typeof paymentAppData.price === "number" && paymentAppData.price > 0) {
    await prisma.credential.findFirstOrThrow({
      where: {
        ...
        ...("credentialId" in paymentAppData && paymentAppData.credentialId ? { id: paymentAppData.credentialId } : { userId: organizerUser.id }),
      },
      select: { id: true },
    });
  }
