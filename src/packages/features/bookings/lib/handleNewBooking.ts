import type {DestinationCalendar} from '~/prisma/app/generated/prisma/client';
...
import processExternalId from '@/packages/app-store/_utils/calendars/processExternalId';
import {metadata as GoogleMeetMetadata} from '@/packages/app-store/googlevideo/_metadata';
import {
  MeetLocationType,
...
//   scheduleTrigger
// } from '@/features/webhooks/lib/scheduleTrigger';
import {getVideoCallUrlFromCalEvent} from '@/packages/lib/CalEventParser';
...
import {getBookerBaseUrl} from '@/packages/lib/getBookerUrl/server';
...
import {
  ...
  getPiiFreeEventType
} from '@/packages/lib/piiFreeData';
import {safeStringify} from '@/lib/safeStringify';
import {
  DistributionMethod,
  getLuckyUser
} from '@/packages/lib/server/getLuckyUser';
...
import {userMetadata as userMetadataSchema} from '~/prisma/zod-utils';
// import { getAllWorkflowsFromEventType } from "@/trpc/server/routers/viewer/workflows/util";
import type {
...
const getEventType = async ({
  ...
  const eventType =
    !eventTypeId && !!eventTypeSlug
      ...
      : await getEventTypesFromDB(eventTypeId);
...
type BookingDataSchemaGetter =
  ...
  | typeof import('@/packages/features/bookings/lib/getBookingDataSchemaForApi').default;
...
function buildLuckyUsersWithJustContactOwner({
  ...
  const isContactOwnerAFixedHostAlready = fixedUserPool.some(
    (user) => user.email === contactOwnerEmail
  );
...
async function handler(
  ...
  } = getLocationValuesForDb(dynamicUserList, users, location);
    ...
    if (!req.body.allRecurringDates || req.body.isFirstRecurringSlot) {
      ...
      while (
        ...
        const newLuckyUser = isSameRoundRobinHost
          ? freeUsers.find(
              (user) => user.id === originalRescheduledBookingUserId
            )
  ...
  try {
    ...
    if (user === null) {
      ...
      finalSyncArgs = {createdDate: createdAt, ...rest};
    }

    await syncServicesUpdateWebUser(finalSyncArgs);
  ...
  if (!eventType.seatsPerTimeSlot && originalRescheduledBooking?.uid) {
    ...
    if (noEmail !== true && isConfirmedByDefault) {
      ...
      loggerWithEventDetails.debug(
        'Emails: Sending rescheduled emails for booking confirmation'
      );
      ...
      if (eventType.schedulingType === SchedulingType.ROUND_ROBIN) {
        ...
        if (originalRescheduledBooking.user) {
          const translate = await getTranslation(
            originalRescheduledBooking.user.locale ?? 'en',
            'common'
          );
          originalBookingMemberEmails.push({
        ...
        const matchOriginalMemberWithNewMember = (
          originalMember: Person,
          newMember: Person
        ) => {
        ...
        const cancelledMembers = originalBookingMemberEmails.filter(
          (member) =>
            !newBookingMemberEmails.find((newMember) =>
              matchOriginalMemberWithNewMember(member, newMember)
            )
        );
        // rescheduled Emails
