...
import {SMS_REMINDER_NUMBER_FIELD} from '@/packages/features/bookings/lib/SystemField';
import type {Workflow} from '@/packages/core/workflows/lib/types';
import {fieldsThatSupportLabelAsSafeHtml} from '@/packages/features/form-builder/fieldsThatSupportLabelAsSafeHtml';
import {getFieldIdentifier} from '@/packages/features/form-builder/utils/getFieldIdentifier';
...
if (typeof window !== 'undefined') {
  ...
  throw new Error(
    '`getBookingFields` must not be imported on the client side.'
  );
}
...
export const ensureBookingInputsHaveSystemFields = ({
  ...
  const smsNumberSources = [] as NonNullable<
    (typeof bookingFields)[number]['sources']
  >;
  ...
  const isEmailFieldOptional = !!bookingFields.find(
    (field) => field.name === 'email' && !field.required
  );
