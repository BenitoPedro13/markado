...
import { getRoutedTeamMemberIdsFromSearchParams } from "@/packages/features/bookings/lib/getRoutedTeamMemberIdsFromSearchParams";
...
export const mapBookingToMutationInput = ({
  ...
  const routingFormResponseIdParam = searchParams.get("markado.routingFormResponseId");
  const routingFormResponseId = routingFormResponseIdParam ? Number(routingFormResponseIdParam) : undefined;
  const skipContactOwner = searchParams.get("markado.skipContactOwner") === "true";
  const reroutingFormResponses = searchParams.get("markado.reroutingFormResponses");
  return {
    ...values,
    user: username,
