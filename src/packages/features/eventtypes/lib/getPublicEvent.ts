import type {User as UserType} from '~/prisma/app/generated/prisma/client';
import {Prisma} from '~/prisma/app/generated/prisma/client';

import type {LocationObject} from '@/core/locations';
import {privacyFilteredLocations} from '@/core/locations';
// import { getAppFromSlug } from "@/app-store/utils";
import dayjs from '@/lib/dayjs';
import {getBookingFieldsWithSystemFields} from '@/packages/features/bookings/lib/getBookingFields';
// import { getSlugOrRequestedSlug } from "@/features/core/organizations/lib/orgDomains";
import {isRecurringEvent, parseRecurringEvent} from '@/packages/lib';
// import { getOrgOrTeamAvatar } from "@/lib/defaultAvatarImage";
import {getPlaceholderAvatar} from '@/packages/lib/defaultAvatarImage';
import {getDefaultEvent, getUsernameList} from '@/packages/lib/defaultEvents';
...
import type {BookerLayoutSettings} from '~/prisma/zod-utils';
import {
  BookerLayouts,
...
export async function isCurrentlyAvailable({
  ...
  const dateOverride = res.availability.find(
    (a) => a.date && dayjs(a.date).isSame(now, 'day')
  );
  ...
  for (const availability of res.availability) {
    if (!availability.date && availability.days.includes(currentDay)) {
      const isAvailable = isAvailableInTimeSlot(
        availability,
        now,
        meetingEndTime
      );
...
export const getPublicEvent = async (
  ...
  if (usernameList.length > 1) {
    ...
    let locations = [] as LocationObject[];
    ...
    const unPublishedOrgUser = users.find(
      (user) => user.profile?.organization?.slug === null
    );
    ...
    return {
      ...
      entity: {
        considerUnpublished:
          !fromRedirectOfNonOrgLink && unPublishedOrgUser !== undefined,
  ...
  const enrichedUsers =
    await UserRepository.enrichUsersWithTheirProfiles(usersAsHosts);
  ...
  if (
    !eventWithUserProfiles.schedule &&
    eventWithUserProfiles.owner?.defaultScheduleId
  ) {
  ...
  if (
    eventWithUserProfiles.isInstantEvent &&
    eventWithUserProfiles.instantMeetingSchedule?.id
  ) {
  ...
  return {
    ...
    bookerLayouts: bookerLayoutsSchema.parse(
      eventMetaData?.bookerLayouts || null
    ),
    ...
    entity: {
      ...
      orgSlug: org,
      teamSlug:
        (eventWithUserProfiles.team?.slug || teamMetadata?.requestedSlug) ??
        null,
      name:
...
function getProfileFromEvent(event: Event) {
  ...
  return {
    ...
    //     }),
    image: nonTeamprofile.image,
    brandColor: profile.brandColor,
    darkBrandColor: profile.darkBrandColor,
    theme: profile.theme,
    bookerLayouts: bookerLayoutsSchema.parse(
      eventMetaData?.bookerLayouts ||
        (userMetaData && 'defaultBookerLayouts' in userMetaData
          ? userMetaData.defaultBookerLayouts
          : null)
    )
  };
}
async function getUsersFromEvent(
  ...
  const {username, name, weekStart, profile, image} = owner;
...
async function getOwnerFromUsersArray(
  ...
  const enrichedUsers =
    await UserRepository.enrichUsersWithTheirProfiles(users);
  ...
  return [
    {
      ...
      bookerUrl: getBookerBaseUrlSync(
        usersWithUserProfile[0].organization?.slug ?? null
      )
    }
  ];
}
...
function mapHostsToUsers(host: {
  user: Pick<UserType, 'username' | 'name' | 'weekStart' | 'image'> & {
    profile: UserProfile;
  };
}) {
  return {
    ...
    bookerUrl: getBookerBaseUrlSync(
      host.user.profile?.organization?.slug ?? null
    ),
