...
import {getDailyAppKeys} from '@/packages/app-store/dailyvideo/lib/getDailyAppKeys';
import {fetcher} from '@/packages/lib/dailyApiFetcher';
...
import type {
  VideoApiAdapter,
  VideoCallData
} from '@/packages/types/VideoApiAdapter';
...
export const getBatchProcessorJobAccessLink = (id: string) => {
  return fetcher(`/batch-processor/${id}/access-link`).then(
...
export const getRoomNameFromRecordingId = (recordingId: string) => {
  return fetcher(`/recordings/${recordingId}`)
    ...
    .then((res: {room_name: string}) => res.room_name);
};
...
export const updateMeetingTokenIfExpired = async ({
  ...
  try {
    await fetcher(`/meeting-tokens/${meetingToken}`).then(
      ZGetMeetingTokenResponseSchema.parse
    );
  } catch (err) {
...
export const generateGuestMeetingTokenFromOwnerMeetingToken = async (
  ...
  const token = await fetcher(`/meeting-tokens/${meetingToken}`).then(
    ZGetMeetingTokenResponseSchema.parse
  );
...
const DailyVideoApiAdapter = (): VideoApiAdapter => {
  ...
  const translateEvent = async (event: CalendarEvent) => {
    ...
    const hasTeamPlan = await prisma.membership.findFirst({
      where: {
        userId: event.organizer.id as string | undefined,
        team: {
          slug: {
            not: null
          }
  ...
  async function createInstantMeeting(endTime: string) {
    ...
    const dailyEvent = await postToDailyAPI('/rooms', body).then(
      dailyReturnTypeSchema.parse
    );
    const meetingToken = await postToDailyAPI('/meeting-tokens', {
      properties: {
  ...
  return {
    ...
    createInstantCalVideoRoom: (endTime: string) =>
      createInstantMeeting(endTime),
    ...
    getAllTranscriptsAccessLinkFromRoomName: async (
      roomName: string
    ): Promise<Array<string>> => {
      try {
        const res = await fetcher(`/rooms/${roomName}`).then(getRooms.parse);
        const roomId = res.id;
        const allTranscripts = await fetcher(
          `/transcript?roomId=${roomId}`
        ).then(getTranscripts.parse);

        if (!allTranscripts.data.length) return [];

        const allTranscriptsIds = allTranscripts.data.map(
          (transcript: {transcriptId: string}) => transcript.transcriptId
        );
        const allTranscriptsAccessLink =
          await processTranscriptsInBatches(allTranscriptsIds);
    ...
    getAllTranscriptsAccessLinkFromMeetingId: async (
      meetingId: string
    ): Promise<Array<string>> => {
      try {
        const allTranscripts = await fetcher(
          `/transcript?mtgSessionId=${meetingId}`
        ).then(getTranscripts.parse);
    ...
    submitBatchProcessorJob: async (
      body: batchProcessorBody
    ): Promise<TSubmitBatchProcessorJobRes> => {
      try {
        const batchProcessorJob = await postToDailyAPI(
          '/batch-processor',
          body
        ).then(ZSubmitBatchProcessorJobRes.parse);
    ...
    getTranscriptsAccessLinkFromRecordingId: async (
      ...
      TGetTranscriptAccessLink['transcription'] | {message: string}
    > => {
      try {
        const batchProcessorJobs = await fetcher(
          `/batch-processor?recordingId=${recordingId}`
        ).then(getBatchProcessJobs.parse);
        if (!batchProcessorJobs.data.length) {
          return {
            message: `No Batch processor jobs found for recording id ${recordingId}`
          };
        }

        const transcriptJobId = batchProcessorJobs.data.filter(
          (job: {preset: string; status: string}) => job.preset === 'transcript' && job.status === 'finished'
        )?.[0]?.id;

        if (!transcriptJobId) return [];

        const accessLinkRes =
          await getBatchProcessorJobAccessLink(transcriptJobId);
    ...
    checkIfRoomNameMatchesInRecording: async (
      roomName: string,
      recordingId: string
    ): Promise<boolean> => {
      try {
        const recording = await fetcher(`/recordings/${recordingId}`).then(
          recordingItemSchema.parse
        );
