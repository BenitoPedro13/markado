...
import { CREDENTIAL_SYNC_ENDPOINT } from "@/packages/app-store/googlecalendar/lib/constants";
...
import { OAuth2UniversalSchemaWithBackwardCompatibility } from "./universalSchema";
...
type UpdateTokenObject = (
  token: z.infer<typeof OAuth2UniversalSchemaWithBackwardCompatibility>
) => Promise<void>;
...
export class OAuthManager {
  ...
  constructor({
    ...
    isTokenExpired = (
      token: z.infer<typeof OAuth2TokenResponseInDbWhenExistsSchema>
    ) => {
      log.debug(
  ...
  public async getTokenObjectOrFetch() {
    ...
    if (!isExpired) {
      ...
      return {
        token: this.normalizeNewlyReceivedToken(this.currentTokenObject),
        isUpdated: false
      };
    } else {
      const token = {
  ...
  public async request<T>(
    ...
    if (typeof customFetchOrUrlAndOptions === 'function') {
      ...
      myLog.debug(
        ...
        safeStringify({customFetchOrUrlAndOptions, headers})
      );
  ...
  private normalizeNewlyReceivedToken(
    ...
    if (!token.expiry_date && !token.expires_in) {
      ...
      token.expiry_date = this.useCredentialSync
  ...
  private async refreshOAuthToken() {
    ...
    if (this.resourceOwner.id && this.useCredentialSync) {
      if (
        ...
        throw new Error(
          'Credential syncing is enabled but the required env variables are not set'
        );
      }
      myLog.debug(
        ...
        response = await fetch(
          `${this.credentialSyncVariables.CREDENTIAL_SYNC_ENDPOINT}`,
          {
            ...
            body: new URLSearchParams({
              markadoUserId: this.resourceOwner.id.toString(),
              appSlug: this.appSlug
            })
          }
        );
    ...
    const parsedToken =
      OAuth2UniversalSchemaWithBackwardCompatibility.safeParse(json);
    if (!parsedToken.success) {
      myLog.error(
        'Token parsing error:',
        safeStringify(parsedToken.error.issues)
      );
  ...
  private async getAndValidateOAuth2Response({response}: {response: Response}) {
    ...
    const tokenObjectUsabilityRes = await this.isTokenObjectUnusable(
      response.clone()
    );
    const accessTokenUsabilityRes = await this.isAccessTokenUnusable(
      response.clone()
    );
