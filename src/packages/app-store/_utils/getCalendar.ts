...
import type { Calendar, CalendarClass } from "@/types/Calendar";
...
export const getCalendar = async (credential: CredentialPayload | null): Promise<Calendar | null> => {
  ...
  if (!calendarAppImportFn) {
    console.warn(`calendar of type ${calendarType} is not implemented`);
    return null;
  }
  ...
  console.info('Got calendarApp', calendarApp.lib.CalendarService);
