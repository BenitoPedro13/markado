...
import { MeetLocationType } from "@/core/locations";
import dayjs from "@/lib/dayjs";
// import { getFeatureFlag } from "@/packages/features/flags/server/utils";
import { getLocation, getRichDescription } from "@/packages/lib/CalEventParser";
import type CalendarService from "@/packages/lib/CalendarService";
import {
  APP_CREDENTIAL_SHARING_ENABLED,
  CREDENTIAL_SYNC_ENDPOINT,
  CREDENTIAL_SYNC_SECRET,
  CREDENTIAL_SYNC_SECRET_HEADER_NAME,
} from "@/packages/app-store/googlecalendar/lib/constants";
import { formatCalEvent } from "@/packages/lib/formatCalendarEvent";
import { getAllCalendars } from "@/packages/lib/google";
...
export default class GoogleCalendarService implements Calendar {
    ...
    const auth = new OAuthManager({
      ...
      updateTokenObject: async (token) => {
        ...
        const {key} = await prisma.credential.update({
          ...
          data: {
            key: token as Prisma.InputJsonValue
          }
        });
    ...
    try {
      ...
      if (formattedCalEvent.existingRecurringEvent) {
        ...
        if (recurringEventInstances.data.items) {
          const markadoEventStartTime = dayjs(formattedCalEvent.startTime)
          ...
          for (let i = 0; i < recurringEventInstances.data.items.length; i++) {
            ...
            if (instanceStartTime === markadoEventStartTime) {
              event = instance;
              break;
            }
          }
  ...
  async deleteEvent(uid: string, event: CalendarEvent, externalCalendarId?: string | null): Promise<void> {
    ...
    try {
      ...
      /**
       *  410 is when an event is already deleted on the Google cal before on markado.co
  ...
  async getCacheOrFetchAvailability(args: {
    ...
    const calendarCacheEnabled = false
