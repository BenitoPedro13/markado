...
import {RiEyeOffLine, RiEyeLine, RiSearchLine} from '@remixicon/react';
            ...
            <Tooltip.Trigger asChild>
              ...
              >
                {isPasswordVisible ? (
                  ...
                  <RiEyeLine className="h-4 w-4 stroke-[2.5px]" />
                )}
                <span className="sr-only">{textLabel}</span>
              </button>
            </Tooltip.Trigger>
            <Tooltip.Content size="small">{textLabel}</Tooltip.Content>
          </Tooltip.Root>
        }
      />
    );
  ...
  return (
    <div>
      ...
      {methods?.formState?.errors[props.name]?.message && (
        ...
        // />
        <Alert.Root status="warning" className="mt-1">{methods.formState.errors[props.name]?.message as string}</Alert.Root>
      )}
    </div>
  );
});
...
export const CalendarSearchField = forwardRef<
  HTMLInputElement,
  InputFieldProps
>(function TextField(props, ref) {
  return (
    <div dir="ltr" className="">
      ...
      <Input ref={ref} className="border-none" {...props} />
    </div>
  );
});
