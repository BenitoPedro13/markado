...
export async function getAllCalendars(
  ...
  try {
    do {
      ...
      console.error('Google Calendar pagination failed in the middle', {
    ...
    console.error("Error fetching all Google Calendars", { error });
    throw error;
  }
}

export async function updateProfilePhoto(oAuth2Client: OAuth2Client, userId: string) {
  try {
    ...
    console.error(
      'Error updating avatarUrl from google calendar connect',
      error
    );
  }
}
