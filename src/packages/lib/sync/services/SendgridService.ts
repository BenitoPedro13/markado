...
// Markado.co Custom Contact Fields
const markadoCoCustomContactFields: SendgridFieldOptions = [
...
export default class SendgridService extends SyncServiceCore implements ISyncService {
  ...
  upsert = async (user: WebUserInfoType | ConsoleUserInfoType) => {
    ...
    const customFieldsIds = await this.service.getSendgridCustomFieldsIds(markadoCoCustomContactFields);
