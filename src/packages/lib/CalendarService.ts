...
import sanitizeCalendarObject from "@/packages/lib/sanitizeCalendarObject";
import type { Person as AttendeeInCalendarEvent } from "@/types/Calendar";
import type {
  Calendar,
  CalendarEvent,
  CalendarEventType,
...
export default abstract class BaseCalendarService implements Calendar {
   ...
   */
  getUserTimezoneFromDB = async (id: string): Promise<string | undefined> => {
   ...
   * @returns {string | null} - The user ID associated with the first calendar in the array, or null if the array is empty or the user ID is not found.
   */
  getUserId = (selectedCalendars: IntegrationCalendar[]): string | null => {
