...
// import type { PaymentPageProps } from "@/ee/payments/pages/payment";
...
export const getBookingRedirectExtraParams = (
  ...
  function extractAttendeesAndGuests(
    booking: SuccessRedirectBookingType,
    obj: ResultType
  ): ResultType {
    ...
    const attendeeStartTime = dayjs(booking.startTime)
      .tz(attendeeTimeZone)
      .format();
  ...
  const queryCompatibleParams: Record<
    string,
...
export const useBookingSuccessRedirect = () => {
  ...
  const bookingSuccessRedirect = ({
    ...
    return router.push(
      `/booking/${booking.uid}${isEmbed ? '/embed' : ''}?${newSearchParams.toString()}`
    );
  };
