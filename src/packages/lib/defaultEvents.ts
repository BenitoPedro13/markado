...
export const getDynamicEventDescription = (
  dynamicUsernames: string[],
  slug: string
): string => {
  return `Book a ${slug} min event with ${dynamicUsernames.join(', ')}`;
};

export const getDynamicEventName = (
  dynamicNames: string[],
  slug: string
): string => {
...
export const getUsernameSlugLink = ({
  users,
  slug
}: UsernameSlugLinkProps): string => {
  let slugLink = ``;
  if (users.length > 1) {
...
export const getUsernameList = (
  ...
  const allUsers = users
    .map((user) => user.replace(/( |%20|%2b)/g, '+').split('+'))
    .flat();
  return Array.prototype.concat(
    ...allUsers.map((userSlug) => slugify(userSlug))
  );
};
