import type { User as PrismaUser } from "~/prisma/app/generated/prisma/client";
...
import type { UpId, UserAsPersonalProfile, UserProfile } from "@/types/UserProfile";
...
export class ProfileRepository {
  ...
  private static getInheritedDataFromUser({
    ...
    user: Pick<PrismaUser, "name" | "image" | "bufferTime">;
  }) {
    return {
      name: user.name,
      image: user.image,
  ...
  private static async _create({
    ...
    email: string;
    movedFromUserId?: string;
  }) {
    console.log("_create", safeStringify({ userId, organizationId, username, email }));
  ...
  static delete({ userId, organizationId }: { userId: string; organizationId: number }) {
  ...
  static deleteMany({ userIds }: { userIds: string[] }) {
    // Even though there can be just one profile matching a userId and organizationId, we are using deleteMany as it won't error if the profile doesn't exist
    return prisma.profile.deleteMany({
  ...
  static async findByUpId(upId: string) {
    ...
    console.log("findByUpId", safeStringify({ upId, lookupTarget }));
  ...
  static async findManyByOrgSlugOrRequestedSlug({
    ...
    console.log("findManyByOrgSlugOrRequestedSlug", safeStringify({ usernames, orgSlug }));
  ...
  static async findManyForUsers(userIds: string[]) {
    const profiles = await prisma.profile.findMany({
      where: {
        userId: {
          in: userIds,
  ...
  static async findManyForUser(user: { id: string }) {
    const profiles = (
  ...
  static async findByUserIdAndProfileId({ userId, profileId }: { userId: string; profileId: number }) {
    const profile = await prisma.profile.findUnique({
      where: {
        userId,
        id: profileId,
