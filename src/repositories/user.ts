...
import { whereClauseForOrgWithSlugOrRequestedSlug } from "@/lib/orgDomains";
...
import type { UpId, UserProfile } from "@/types/UserProfile";

import { DEFAULT_SCHEDULE, getAvailabilityFromSchedule, getAvailabilityFromScheduleWithoutUser } from "@/lib/availability";
...
export class UserRepository {
  ...
  static async findUsersByUsername({
    ...
    return (
      ...
      if (!profile) {
        console.error("Profile not found for user", safeStringify({ user, profiles }));
  ...
  static async findByIds({ ids }: { ids: string[] }) {
  ...
  static async findByIdOrThrow({ id }: { id: string }) {
  ...
  static async findIfAMemberOfSomeOrganization({ user }: { user: { id: string } }) {
    return !!(
  ...
  static async enrichUserWithTheProfile<T extends { username: string | null; id: string }>({
    user,
    upId,
  }: {
    user: T;
   ...
   */
  static async enrichUserWithItsProfile<T extends { id: string; username: string | null }>({
    user,
  }: {
    user: T;
  }): Promise<
  ...
  static async enrichUsersWithTheirProfiles<T extends { id: string; username: string | null }>(
    users: T[]
  ): Promise<
    Array<
      T & {
  ...
  > {
    ...
    const personalProfileMap = new Map<string, UserProfile>();
    users.forEach((user) => {
  ...
  static enrichUserWithItsProfileBuiltFromUser<T extends { id: string; username: string | null }>({
    user,
  }: {
    user: T;
  }): T & {
  ...
  static async create({
    ...
    const password = createHash('md5')
      .update(`${email}${process.env.ENCRYPTION_KEY}`)
      .digest('hex');
    const hashedPassword = await hashPassword(password);
    // const t = await getTranslation("en", "common");
    const availability =
      getAvailabilityFromScheduleWithoutUser(DEFAULT_SCHEDULE);
  ...
  static async getUserAdminTeams(userId: string) {
  ...
  static async isAdminOfTeamOrParentOrg({ userId, teamId }: { userId: string; teamId: number }) {
  ...
  static async isAdminOrOwnerOfTeam({ userId, teamId }: { userId: string; teamId: number }) {
  ...
  static async getTimeZoneAndDefaultScheduleId({ userId }: { userId: string }) {
  ...
  static async adminFindById(userId: string) {
  ...
  static async findUserTeams({ id }: { id: string }) {
    const user = await prisma.user.findUnique({
      where: {
        id,
      },
  ...
  static async updateAvatar({ id, avatarUrl }: { id: string; avatarUrl: string }) {
