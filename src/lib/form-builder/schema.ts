...
import {getValidRhfFieldName} from './utils/getFieldIdentifier';
...
export const fieldTypeConfigSchema = z
  ...
  .refine((data) => {
    ...
    if (!variantsConfig.variants[variantsConfig.defaultVariant]) {
      throw new Error(
        `defaultVariant: ${variantsConfig.defaultVariant} is not in variants`
      );
    }
    return true;
  });
...
> = {
  name: {
    preprocess: ({response, field}) => {
      ...
      const variantInResponse =
        field.variant || fieldTypeConfig?.variantsConfig?.defaultVariant;
      ...
      if (
        variantInResponse !== 'firstAndLastName' &&
        variantInResponse !== 'fullName'
      ) {
    ...
    superRefine: ({field, response, isPartialSchema, ctx, m}) => {
      ...
      const fields =
        variantsConfig.variants[
          variantInResponse as keyof typeof variantsConfig.variants
        ].fields;
      ...
      if (fields.length === 1) {
        ...
        if (variantSupportedFields.includes(field.type)) {
          const schema =
            field.required && !isPartialSchema
              ? nonEmptyString()
              : stringSchema;
          ...
          throw new Error(
            `Unsupported field.type with variants: ${field.type}`
          );
        }
      }
      fields.forEach((subField) => {
        const schema =
          subField.required && !isPartialSchema
            ? nonEmptyString()
            : stringSchema;
        if (!variantSupportedFields.includes(subField.type)) {
          throw new Error(
            `Unsupported field.type with variants: ${subField.type}`
          );
        }
  ...
  textarea: {
    ...
    superRefine: ({field, response, ctx, m}) => {
      ...
      const maxLength =
        field.maxLength ?? fieldTypeConfig.supportsLengthCheck?.maxLength;
