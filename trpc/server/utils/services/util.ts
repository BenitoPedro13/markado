...
import type {EventTypeRepository} from '@/repositories/eventType';
import {UserRepository} from '@/repositories/user';
import {MembershipRole, PeriodType} from '~/prisma/enums';
...
//       })
//       .refine(
//         (data) => data.id !== undefined || data.eventTypeId !== undefined,
//         {
...
//         event.users.find((user) => user.id === ctx.user.id)
//       );
//     })();
...
//       if (event.team) {
//         const allTeamMembers = event.team.members.map(
//           (member) => member.userId
//         );
//         return input.users.every((userId: number) =>
//           allTeamMembers.includes(userId)
//         );
//       }
...
export function handlePeriodType(
...
export function handleCustomInputs(
  ...
  const cInputsIdsToDeleteOrUpdated = customInputs.filter(
    (input) => !input.hasToBeCreated
  );
...
export const mapEventType = async (eventType: EventType) => ({
  ...eventType,
  safeDescription: eventType?.description
    ? 
    // markdownToSafeHTML(
  ...
  metadata: eventType.metadata
    ? EventTypeMetaDataSchema.parse(eventType.metadata)
    : null,
