...
import {getAggregatedAvailability} from '@/packages/core/getAggregatedAvailability';
import {getBusyTimesForLimitChecks} from '@/packages/core/getBusyTimes';
import type {
  CurrentSeats,
  IFromUser,
  IToUser,
  GetAvailabilityUser
} from '@/packages/core/getUserAvailability';
import {getUsersAvailability} from '@/packages/core/getUserAvailability';
// import monitorCallbackAsync, { monitorCallbackSync } from "@/core/sentryWrapper";
import type {Dayjs} from '@/lib/dayjs';
import dayjs from '@/lib/dayjs';
import { getSlugOrRequestedSlug, orgDomainConfig } from "@/packages/features/ee/organizations/lib/orgDomains";
import {isEventTypeLoggingEnabled} from '@/packages/features/bookings/lib/isEventTypeLoggingEnabled';
import {parseBookingLimit, parseDurationLimit} from '@/packages/lib';
import {getRoutedHostsWithContactOwnerAndFixedHosts} from '@/packages/features/bookings/lib/getRoutedUsers';
import {RESERVED_SUBDOMAINS} from '@/constants';
import {getUTCOffsetByTimezone} from '@/lib/date-fns';
import {getDefaultEvent} from '@/packages/lib/defaultEvents';
import {
  isTimeOutOfBounds,
  calculatePeriodLimits,
  isTimeViolatingFutureLimit
} from '@/packages/lib/isOutOfBounds';
...
import type {TGetScheduleInputSchema} from '~/trpc/server/schemas/public/getSchedule.schema';
...
function applyOccupiedSeatsToCurrentSeats(
  ...
  const occupiedSeatsCount = countBy(occupiedSeats, (item) =>
    item.slotUtcStartDate.toISOString()
  );
...
export function getUsersWithCredentialsConsideringContactOwner({
  ...
  const contactOwnerHost = hosts.find(
    (host) => host.user.email === contactOwnerEmail
  );
  /**
...
const getStartTime = (
  ...
  const startTime =
    timeZone === 'Etc/GMT'
      ...
      : dayjs(startTimeInput).tz(timeZone);

  return startTimeMin.isAfter(startTime)
    ? startTimeMin.tz(timeZone)
    : startTime;
};
...
async function _getAvailableSlots({
  input,
  ctx
}: GetScheduleOptions): Promise<IGetAvailableSlots> {
  const {
  ...
  const orgDetails = input?.orgSlug
    ? {
        ...
        isValidOrgDomain:
          !!input.orgSlug && !RESERVED_SUBDOMAINS.includes(input.orgSlug)
      }
  ...
  const isRollingWindowPeriodType =
    eventType.periodType === PeriodType.ROLLING_WINDOW;
  const startTimeAsIsoString = input.startTime;
  const isStartTimeInPast = dayjs(startTimeAsIsoString).isBefore(
  ...
  const loggerWithEventDetails = logger.getSubLogger({
    prefix: [
      'getAvailableSlots',
      `${eventType.id}:${input.usernameList}/${input.eventTypeSlug}`
    ]
  });
  ...
  if (!startTime.isValid() || !endTime.isValid()) {
    throw new TRPCError({
      message: 'Invalid time range given.',
      code: 'BAD_REQUEST'
    });
  }
  ...
  const contactOwnerEmail = skipContactOwner
    ? null
    : contactOwnerEmailFromInput;

  const routedHostsWithContactOwnerAndFixedHosts =
  ...
  if (contactOwnerEmail && aggregatedAvailability.length > 0) {
    ...
    if (diff > 0) {
      const routedHostsAndFixedHosts =
        routedHostsWithContactOwnerAndFixedHosts.filter(
  ...
  await prisma.selectedSlots.deleteMany({
    where: {
      ...
      id: {notIn: selectedSlots.map((item) => item.id)}
    }
  });
    ...
    if (occupiedSeats?.length) {
      ...
      if (typeof availabilityCheckProps.currentSeats !== 'undefined') {
        availabilityCheckProps.currentSeats =
          availabilityCheckProps.currentSeats.map((item) => {
            const attendees =
              occupiedSeats.filter(
                (seat) =>
                  seat.slotUtcStartDate.toISOString() ===
                  item.startTime.toISOString()
              )?.length || 0;
            if (attendees)
              addedToCurrentSeats.push(item.startTime.toISOString());
            return {
              ...item,
              _count: {
        ...
        occupiedSeats = occupiedSeats.filter(
          (item) =>
            !addedToCurrentSeats.includes(item.slotUtcStartDate.toISOString())
        );
      }
  ...
  const slotsMappedToDate = (function mapSlotsToDate() {
    return availableTimeSlots.reduce(
      (
        r: Record<
          string,
  ...
  // const allDatesWithBookabilityStatus = monitorCallbackSync(
  //   getAllDatesWithBookabilityStatus,
  //   availableDates
  // );
  const allDatesWithBookabilityStatus =
  ...
  // const withinBoundsSlotsMappedToDate = monitorCallbackSync(
  //   function mapWithinBoundsSlotsToDate() {
  //     return Object.entries(slotsMappedToDate).reduce(
  ...
  if (
    !Object.keys(withinBoundsSlotsMappedToDate).length &&
    input.usernameList?.length === 1
  ) {
    try {
...
async function getUserIdFromUsername(
  ...
  log.info(
    ...
    safeStringify({organizationDetails, username})
  );
...
async function getExistingBookings(
  ...
  usersWithCredentials: ReturnType<
    typeof getUsersWithCredentialsConsideringContactOwner
  >,
  allUserIds: string[]
) {
...
const calculateHostsAndAvailabilities = async ({
  ...
  input: TGetScheduleInputSchema;
  eventType: Exclude<
    Awaited<ReturnType<typeof getRegularOrDynamicEventType>>,
    null
  >;
  routedHostsWithContactOwnerAndFixedHosts: {
    isFixed?: boolean;
  ...
  // const usersWithCredentials = monitorCallbackSync(
  //   getUsersWithCredentialsConsideringContactOwner,
  //   {
  //     contactOwnerEmail,
  ...
  const endTimeDate =
    input.rescheduleUid && durationToUse
      ? endTime.add(durationToUse, 'minute').toDate()
      : endTime.toDate();
  ...
  //   allUserIds
  // );
  const currentBookingsAllUsers = await getExistingBookings(
    startTimeDate,
    endTimeDate,
    eventType,
    sharedQuery,
  ...
  const outOfOfficeDaysAllUsers = await getOOODates(
    startTimeDate,
    endTimeDate,
    allUserIds
  );
  ...
  let busyTimesFromLimitsBookingsAllUsers: Awaited<
    ReturnType<typeof getBusyTimesForLimitChecks>
  > = [];

  if (eventType && (bookingLimits || durationLimits)) {
    // busyTimesFromLimitsBookingsAllUsers = await monitorCallbackAsync(
  ...
  //             b.attendees?.some((a) => a.email === currentUser.email)
  //         )
  //         .map((bookings) => {
  //           const {attendees: _attendees, ...bookingWithoutAttendees} =
  //             bookings;
  //           return bookingWithoutAttendees;
  //         }),
  //       outOfOfficeDays: outOfOfficeDaysAllUsers.filter(
  //         (o) => o.user.id === currentUser.id
  //       )
  //     };
  //   });
  // });

  const users = function enrichUsersWithData() {
    return usersWithCredentials.map((currentUser) => {
      return {
        ...currentUser,
        currentBookings: currentBookingsAllUsers
