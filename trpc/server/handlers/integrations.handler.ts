...
import type {TDependencyData} from '@/packages/app-store/_appRegistry';
import type {CredentialOwner} from '@/packages/app-store/types';
import {getAppFromSlug} from '@/packages/app-store/utils';
import getEnabledAppsFromCredentials from '@/packages/lib/apps/getEnabledAppsFromCredentials';
import getInstallCountPerApp from '@/packages/lib/apps/getInstallCountPerApp';
import {getUsersCredentials} from '~/trpc/server/lib/getUsersCredentials';
...
import type {PaymentApp} from '@/packages/types/PaymentService';

import type {TIntegrationsInputSchema} from '~/trpc/server/schemas/integrations.schema';
...
export type TeamQuery = Prisma.TeamGetPayload<{
  select: {
    id: true;
    credentials: {
      select: typeof import('~/prisma/selects/credential').credentialForCalendarServiceSelect;
    };
    name: true;
    logoUrl: true;
    members: {
...
export const integrationsHandler = async ({input}: IntegrationsOptions) => {
  ...
  let credentials = await getUsersCredentials(session?.user!);
  ...
  if (includeTeamInstalledApps || teamId) {
    ...
    const teamAppCredentials: CredentialPayload[] = userTeams.flatMap(
      (teamApp) => {
      ...
      async ({
        ...
        if (credential && app.categories.includes('payment')) {
          ...
          if (
            ...
            'lib' in paymentApp &&
            paymentApp?.lib &&
            'PaymentService' in paymentApp?.lib
          ) {
        ...
        return {
          ...
          teams,
          isInstalled:
            !!userCredentialIds.length || !!teams.length || app.isGlobal,
          isSetupAlready,
  ...
  if (onlyInstalled) {
    apps = apps.flatMap((item) =>
      item.userCredentialIds.length > 0 || item.teams.length || item.isGlobal
        ? [item]
        : []
    );
  }
  ...
  if (extendsFeature) {
    apps = apps
      ...
      .map((app) => ({
        ...app,
        isInstalled:
          !!app.userCredentialIds?.length || !!app.teams?.length || app.isGlobal
      }));
  }
