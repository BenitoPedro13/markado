...
import {isSupportedTimeZone} from '@/lib/date-fns';
import {
  emailSchema as emailRegexSchema,
  emailRegex
} from '@/lib/emailSchema';
import {slugify} from '@/lib/slugify';
import {EventTypeCustomInputType} from '~/prisma/enums';
...
export const EventTypeMetaDataSchema = z
  .object({
    ...
    managedEventConfig: z
      .object({
        unlockedFields: z
          .custom<{[k in keyof Omit<Prisma.EventTypeSelect, 'id'>]: true}>()
          .optional()
      })
      .optional(),
...
export const BookingFieldTypeEnum =
  eventTypeBookingFields.element.shape.type.Enum;
...
export const bookingCreateBodySchema = z.object({
  ...
  timeZone: z
    .string()
    .refine((value: string) => isSupportedTimeZone(value), {
...
export const bookingCreateSchemaLegacyPropsForApi = z.object({
  ...
  customInputs: z.array(
    z.object({label: z.string(), value: z.union([z.string(), z.boolean()])})
  )
});
...
export type DefaultConferencingApp = z.infer<
  typeof schemaDefaultConferencingApp
>;
...
export type GetRecordingsResponseSchema = z.infer<
  typeof getRecordingsResponseSchema
>;
...
export const getParserWithGeneric =
  ...
  <Data>(data: Data) => {
    ...
    return valueSchema.parse(data) as {
      ...
      [key in keyof Data]: Data[key] extends SimpleFormValues
        ? Data[key]
        : Output[key];
    };
  };
...
export const allManagedEventTypeProps: {
  [k in keyof Omit<Prisma.EventTypeSelect, 'id'>]: true;
} = {
  title: true,
  description: true,
  isInstantEvent: true,
